...
void Prostopadloscian::get_global_wspolrzedne (Wektor3D wspolrzedne[])
{
    ...
    wspolrzedne[1] = Odniesienie + tmp - OrX;
    wspolrzedne[2] = Odniesienie + tmp - OrX - OrY;
    wspolrzedne[3] = Odniesienie + tmp - OrY;
    wspolrzedne[4] = Odniesienie + tmp - OrZ;
    wspolrzedne[5] = Odniesienie + tmp - OrX - OrZ;
    wspolrzedne[6] = Odniesienie + tmp - OrX - OrY - OrZ;
    wspolrzedne[7] = Odniesienie + tmp - OrY - OrZ;
}
...
double Prostopadloscian::wez_bok (char ktory) 
{
  if (ktory=='A')
  return A;
  if (ktory=='B')
